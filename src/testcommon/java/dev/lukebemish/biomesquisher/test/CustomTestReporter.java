...
public class CustomTestReporter implements TestReporter {
    ...
    private Element createTestCase(GameTestInfo testInfo) {
        ...
        var batch = batchTestSuites.computeIfAbsent(testInfo.getTestFunction().batchName(), s -> {
        ...
        var batchTime = testSuiteTimes.getOrDefault(testInfo.getTestFunction().batchName(), 0L);
        testSuiteTimes.put(testInfo.getTestFunction().batchName(), batchTime + testInfo.getRunTime());
