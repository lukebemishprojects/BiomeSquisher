...
public class BiomeSquisherTest {
    public BiomeSquisherTest(IEventBus modBus) throws ParserConfigurationException, IOException {
        modBus.addListener(RegisterGameTestsEvent.class, this::registerTests);

        var testOutputDir = FMLPaths.GAMEDIR.get().resolve("results");
        ...
        public static void fromClass(Class<?> clazz, List<TestFunction> tests) {
            for (var method : clazz.getMethods()) {
                if (method.isAnnotationPresent(GameTest.class)) {
                    ...
                    String testName = method.getDeclaringClass().getSimpleName().toLowerCase(Locale.ROOT) + '.' + method.getName().toLowerCase(Locale.ROOT);
                    String structureName = gameTest.template().isEmpty() ? method.getName().toLowerCase(Locale.ROOT) : gameTest.template();
                    String batchName = gameTest.batch();
